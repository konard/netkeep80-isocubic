...
# План задач: Фаза 4 — Мультиплеер и коллаборация

Данный раздел содержит последовательность ISSUE для реализации мультиплеерной системы и коллаборативного редактирования.
...
**Описание**:
Создать фундаментальную инфраструктуру для поддержки совместной работы над кубиками: типы данных, сессии, синхронизация состояния.

**Задачи**:
- [x] Создать типы для коллаборации `/src/types/collaboration.ts`:
  - `Session` — структура сессии совместной работы
  - `Participant` — участник сессии с ролью
  - `CollaborativeAction` — действие для синхронизации
  - `SessionState` — состояние сессии
- [x] Создать модуль `/src/lib/collaboration.ts`:
  - Управление сессиями (создание, присоединение, выход)
  - Очередь действий для синхронизации
  - Разрешение конфликтов (последний выигрывает)
  - Оптимистичное обновление состояния
- [x] Создать хранилище для коллаборативного состояния:
  - Интеграция с существующим storage модулем
  - Локальный кеш для offline режима
  - Отслеживание изменений участников
- [x] Написать unit-тесты для всех модулей
...
**Задачи**:
- [ ] Создать модуль `/src/lib/websocket-client.ts`:
  - Подключение к серверу с реконнектом
  - Отправка и получение сообщений
  - Heartbeat для поддержания соединения
  - Обработка ошибок соединения
- [ ] Создать протокол сообщений:
  - `join_session` — присоединение к сессии
  - `leave_session` — выход из сессии
  - `sync_action` — синхронизация действия
  - `full_sync` — полная синхронизация состояния
  - `presence_update` — обновление присутствия
- [ ] Интегрировать с collaboration модулем
- [ ] Добавить fallback на polling для устаревших браузеров
...
**Задачи**:
- [ ] Создать компонент `/src/components/SessionPanel.tsx`:
  - Создание новой сессии
  - Присоединение по коду/ссылке
  - Список участников с аватарами
  - Индикаторы статуса (online/offline)
- [ ] Создать компонент `/src/components/ParticipantCursor.tsx`:
  - Отображение курсоров других участников
  - Имена и цвета участников
  - Плавная анимация движения
- [ ] Создать компонент `/src/components/ActionHistory.tsx`:
  - Лента последних действий
  - Фильтрация по участнику
  - Откат действий (для модераторов)
...
### ISSUE 24: Серверная часть (опционально)

**Заголовок**: `Создание серверной части для координации мультиплеера`

**Описание**:
Разработать минимальный сервер для координации сессий (опционально, может использоваться существующий BaaS).

**Задачи**:
- [ ] Оценить варианты реализации:
  - Собственный сервер (Node.js/Deno)
  - Firebase Realtime Database
  - Supabase Realtime
  - PeerJS (P2P без сервера)
- [ ] Реализовать выбранное решение
- [ ] Документировать API и развёртывание
- [ ] Добавить аутентификацию сессий
...
**Метки**: `backend`, `infrastructure`, `optional`

---

## Диаграмма зависимостей Фазы 4
...
| 24. Серверная часть | Высокая | Средний (опционально) |

---

## Следующие шаги

После завершения Фазы 4 планируется:

- **Фаза 5**: Публикация и шаринг кубиков
